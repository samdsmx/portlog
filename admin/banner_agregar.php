...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-casa.jpg" width="35" height="24">Agregar Banner</h1>
        </article>
...
<textarea class="campo" name="descripcion" rows=""></textarea>
...
if (isset($_POST['guardar']))
{
	...
	echo $imagen=$nombre_archivo4;
			
	if ($nombre_archivo4!="")
	{ 
		if ( (strpos($tipo_archivo4 , "jpeg") || strpos($tipo_archivo4 , "png") || strpos($tipo_archivo4 , "jpg") || strpos($tipo_archivo4 , "gif")) ) 
		{
			copy($_FILES['imagen']['tmp_name'], $carpeta.$nombre_archivo4);
			extract($_REQUEST);
			mysqli_select_db($con_admin , $database_con_admin);
			$qry44 = "INSERT INTO banner (imagen, orden, titulo, descripcion) VALUES ('$nombre_archivo4', '$orden', '$titulo', '$descripcion')";
			mysqli_query($con_admin, $qry44) or die(header('location:home.php'));
			echo "<SCRIPT LANGUAJE='javascript'>window.location='banner_actualizar.php' </SCRIPT>";	
		}
		else
		{  
			echo "<script languaje='javascript'>alert('ERROR EN LA IMAGEN')</script>";
			echo"<script language='javascript'>window.location='banner_agregar.php'</script>";	
		}	
	}
	else
	{
