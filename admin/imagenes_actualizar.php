<?php session_start();?>
<!DOCTYPE html>
<html lang="es"><!-- InstanceBegin template="/Templates/General_admin.dwt.php" codeOutsideHTMLIsLocked="false" -->
<?php
...
<script type="text/javascript">
function goBack(){
	location.href="galeria_actualizar.php";
}
</script>
<!-- InstanceEndEditable -->
...
?>  
        	<h1 class="h2"><img src="imagenes/interiores/icono-banner.jpg" width="35" height="24">Imagenes: <strong><?php echo $titulo; ?></strong></h1>
        </article>
        
        	<!--<h2 class="subtitulos">Nueva imagen:</h2>            
            <button class="agregar" onclick="location.href='agregar_galeria.php'">Agregar</button>-->
        ...
        <article id="contenido-admin">
            <h2 class="subtitulos">Nueva imagen:</h2>
            <button class="agregar" onclick="location.href='imagenes_agregar.php?galeria=<?php echo $galeria; ?>'">Agregar</button>
        </article>
        ...
        <article id="contenido-admin">
        	...
        	<h2 class="subtitulos">Tamaño de imagen:</h2>
            <p class="texto">574 x 441 px</p>
        </article>
        
       
        <article id="contenido-admin2">
        	<form action="" method="post" name="formulario" id="formulario" onReset="return goBack();"><input name="enviar" type="reset" class="cancelar" id="enviar" value="Cancelar" /></form>
        </article>
        
        <article id="contenido-admin">
                <h2 class="subtitulos2">Lista de agregados</h2>
               
<?php
mysqli_select_db($con_admin, $database_con_admin);
$query = "SELECT COUNT(id_imagenes) FROM imagenes WHERE galeria='$galeria'";
$result = mysqli_query($con_admin, $query) or die(header('location:home.php'));
$count = mysqli_fetch_row($result);
$pages2= new Paginator; 
$pages2->items_total = $count[0]; //consulta del count
$pages2->mid_range = 4; //cantidad de links a mostrar numero impar 
...
<?php              
mysqli_select_db($con_admin, $database_con_admin);
$qry2 = "SELECT * FROM imagenes WHERE galeria='$galeria' ORDER BY orden ASC $pages2->limit";
$r2 = mysqli_query($con_admin, $qry2) or die(header('location:home.php'));
if ( $r2 !== false && mysqli_num_rows($r2) > 0 ) 
{
	while ($a2 = mysqli_fetch_assoc($r2))
	{
		$id_imagenes = test_input($a2['id_imagenes']);
		$img_imagenes = test_input($a2['imagen']);
		$orden_imagenes = test_input($a2['orden']);
		$titulo = test_input($a2['titulo']);
?>              
                
                    <div id="bloque-actualizar">
                        <div id="bloque-imagen">
                            <p class="texto" style="width:250px;"><?php echo $orden_imagenes;?>.- <?php echo $titulo; ?></p>
                            <figure id="img"><img src="../img/imagenes/<?php echo $img_imagenes;?>" width="200" ></figure>
                        </div>
                        <div id="bloque-acciones">                        
                            <button class="actualizar" onclick="location.href='imagenes_editar.php?id_imagenes=<?php echo $id_imagenes;?>'">Actualizar</button>
							
                            <button class="eliminar" onclick="if (confirm('¿Desea eliminar el imagenes?')==true){window.location.href='imagenes_eliminar.php?id_imagenes=<?php echo $id_imagenes; ?>&galeria=<?php echo $galeria; ?>'}">Eliminar</button>
