...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-banner.jpg" width="35" height="24">galeria</h1>
        </article>
        ...
        <article id="contenido-admin">
            <h2 class="subtitulos">Nueva imagen:</h2>
            <button class="agregar" onclick="location.href='galeria_agregar.php'">Agregar</button>
        </article>
...
<?php
mysqli_select_db($con_admin, $database_con_admin);
$query = "SELECT COUNT(id_galeria) FROM galeria";
...
<?php              
mysqli_select_db($con_admin, $database_con_admin);
$qry2 = "SELECT * FROM galeria ORDER BY orden ASC $pages2->limit";
$r2 = mysqli_query($con_admin, $qry2) or die(header('location:home.php'));
if ( $r2 !== false && mysqli_num_rows($r2) > 0 ) 
{
	while ($a2 = mysqli_fetch_assoc($r2))
	{
		$id_galeria = test_input($a2['id_galeria']);
		$img_galeria = test_input($a2['imagen']);
		$orden_galeria = test_input($a2['orden']);
		$titulo = test_input($a2['titulo']);
?>              
                
                    <div id="bloque-actualizar">
                        <div id="bloque-imagen">
                            <p class="texto" style="width:250px;"><?php echo $orden_galeria;?>.- <?php echo $titulo; ?></p>
                            <figure id="img"><img src="../img/galeria/<?php echo $img_galeria;?>" width="200" ></figure>
                        </div>
                        <div id="bloque-acciones">
                        <button class="actualizar" onclick="location.href='imagenes_actualizar.php?galeria=<?php echo $id_galeria;?>'">Imágenes</button>
                        
                            <button class="actualizar" onclick="location.href='galeria_editar.php?id_galeria=<?php echo $id_galeria;?>'">Actualizar</button>
							
                            <button class="eliminar" onclick="if (confirm('¿Desea eliminar el galeria?')==true){window.location.href='galeria_eliminar.php?id_galeria=<?php echo $id_galeria; ?>'}">Eliminar</button>
