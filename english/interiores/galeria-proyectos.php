...
<?php              
mysqli_select_db($con_admin, $database_con_admin);
$qry2 = "SELECT * FROM galeria ORDER BY orden ASC";
$r2 = mysqli_query($con_admin, $qry2) or die(header('location:home.php'));
if ( $r2 !== false && mysqli_num_rows($r2) > 0 ) 
{
	while ($a2 = mysqli_fetch_assoc($r2))
	{
		...
		$ids = test_input($a2['id_galeria']);
		$titulo = test_input($a2['titulo']);
		$vowels = array(" ");
 $onlyconsonants = str_replace($vowels, "-", $titulo);
?>     	
        <div class="gal_ind col-xl-3 col-mc-4 col-ch-12">
        <a href="galeria-proyectos-interior.php?galeria=<?php echo $ids; ?>&t=<?php echo $onlyconsonants; ?>">
        	<figure><img src="../img/galeria/<?php echo $img_galeria; ?>" title="<?php echo $titulo; ?>" alt="<?php echo $titulo,'-',$img_galeria;?>"><div class="mascara"><span class="icon-mas"></span></div></figure>
