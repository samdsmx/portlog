...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-casa.jpg" width="35" height="24">Agregar galeria</h1>
        </article>
...
{
	extract ($_REQUEST);
	echo"<script language='javascript'>window.location='actualizar_galeria.php'</script>";
}
...
if (isset($_POST['guardar']))
{
	...
	{ 
		if ( (strpos($tipo_archivo4 , "jpeg") || strpos($tipo_archivo4 , "png") || strpos($tipo_archivo4 , "gif")) && ($tamano_imagen < 61400400) ) 
		{
			...
			extract($_REQUEST);
			mysqli_select_db($con_admin , $database_con_admin);
			$qry44 = "INSERT INTO galeria (imagen, titulo, orden) VALUES ('$nombre_archivo4', '$titulo', '$orden')";
			mysqli_query($con_admin, $qry44) or die(header('location:home.php'));
			echo "<SCRIPT LANGUAJE='javascript'>window.location='galeria_actualizar.php' </SCRIPT>";	
		}
		else
		{  
			echo "<script languaje='javascript'>alert('ERROR EN LA IMAGEN')</script>";
			echo"<script language='javascript'>window.location='galeria_agregar.php'</script>";	
		}	
	}
	else
	{
