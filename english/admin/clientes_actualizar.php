...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-banner.jpg" width="35" height="24">clientes</h1>
        </article>
        
        	<!--<h2 class="subtitulos">Nueva imagen:</h2>            
            <button class="agregar" onclick="location.href='agregar_clientes.php'">Agregar</button>-->
        ...
        <article id="contenido-admin">
            <h2 class="subtitulos">Nueva imagen:</h2>
            <button class="agregar" onclick="location.href='clientes_agregar.php'">Agregar</button>
        </article>
...
<?php
mysqli_select_db($con_admin, $database_con_admin);
$query = "SELECT COUNT(id_clientes) FROM clientes";
...
<?php              
mysqli_select_db($con_admin, $database_con_admin);
$qry2 = "SELECT * FROM clientes ORDER BY orden ASC $pages2->limit";
$r2 = mysqli_query($con_admin, $qry2) or die(header('location:home.php'));
if ( $r2 !== false && mysqli_num_rows($r2) > 0 ) 
{
	while ($a2 = mysqli_fetch_assoc($r2))
	{
		$id_clientes = test_input($a2['id_clientes']);
		$img_clientes = test_input($a2['imagen']);
		$orden_clientes = test_input($a2['orden']);
		$titulo = test_input($a2['titulo']);
?>              
                
                    <div id="bloque-actualizar">
                        <div id="bloque-imagen">
                            <p class="texto" style="width:250px;"><?php echo $orden_clientes;?>.- <?php echo $titulo; ?></p>
                            <figure id="img"><img src="../img/clientes/<?php echo $img_clientes;?>" width="200" ></figure>
                        </div>
                        <div id="bloque-acciones">
                            <button class="actualizar" onclick="location.href='clientes_editar.php?id_clientes=<?php echo $id_clientes;?>'">Actualizar</button>
							
                            <button class="eliminar" onclick="if (confirm('¿Desea eliminar el clientes?')==true){window.location.href='clientes_eliminar.php?id_clientes=<?php echo $id_clientes; ?>'}">Eliminar</button>
