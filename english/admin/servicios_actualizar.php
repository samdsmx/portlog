...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-banner.jpg" width="35" height="24">servicios</h1>
        </article>
        
        	<!--<h2 class="subtitulos">Nueva imagen:</h2>            
            <button class="agregar" onclick="location.href='agregar_servicios.php'">Agregar</button>-->
        ...
        <article id="contenido-admin">
            <h2 class="subtitulos">Nueva imagen:</h2>
            <button class="agregar" onclick="location.href='servicios_agregar.php'">Agregar</button>
        </article>
...
<?php
mysqli_select_db($con_admin, $database_con_admin);
$query = "SELECT COUNT(id_servicios) FROM servicios";
...
$pages2->mid_range = 5; //cantidad de links a mostrar numero impar 
...
if ( $r2 !== false && mysqli_num_rows($r2) > 0 ) 
{
	...
	{
		$id_servicios = test_input($a2['id_servicios']);
		$img_servicios = test_input($a2['imagen']);
		$orden_servicios = test_input($a2['orden']);
		$titulo = test_input($a2['titulo']);
?>              
                
                    <div id="bloque-actualizar">
                        <div id="bloque-imagen">
                            <p class="texto" style="width:250px;"><?php echo $orden_servicios;?>.- <?php echo $titulo; ?></p>
                            <figure id="img"><img src="../img/servicios/<?php echo $img_servicios;?>" width="200" ></figure>
                        </div>
                        <div id="bloque-acciones">
                            <button class="actualizar" onclick="location.href='servicios_editar.php?id_servicios=<?php echo $id_servicios;?>'">Actualizar</button>
							
                            <button class="eliminar" onclick="if (confirm('¿Desea eliminar el servicios?')==true){window.location.href='servicios_eliminar.php?id_servicios=<?php echo $id_servicios; ?>'}">Eliminar</button>
