...
<script type="text/javascript">
function goBack(){
	location.href="imagenes_actualizar.php?galeria=<?php echo $galeria; ?>";
}
</script>
...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-casa.jpg" width="35" height="24">Agregar imagenes</h1>
        </article>
...
if (isset($_POST['guardar']))
{
	...
	if ($nombre_archivo4!="")
	{ 
		if ( (strpos($tipo_archivo4 , "jpeg") || strpos($tipo_archivo4 , "png") || strpos($tipo_archivo4 , "gif"))  ) 
		{
			copy($_FILES['imagen']['tmp_name'], $carpeta.$nombre_archivo4);
			extract($_REQUEST);
			mysqli_select_db($con_admin , $database_con_admin);
			$qry44 = "INSERT INTO imagenes (imagen, titulo, orden, galeria) VALUES ('$nombre_archivo4', '$titulo', '$orden', '$galeria')";
			mysqli_query($con_admin, $qry44) or die(header('location:home.php'));
			echo "<SCRIPT LANGUAJE='javascript'>window.location='imagenes_actualizar.php?galeria=".$galeria."' </SCRIPT>";	
		}
		else
		{  
			echo "<script languaje='javascript'>alert('ERROR EN LA IMAGEN')</script>";
			echo"<script language='javascript'>window.location='imagenes_agregar.php?galeria=".$galeria."'</script>";	
		}	
	}
	else
	{
