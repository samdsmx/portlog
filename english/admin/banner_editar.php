...
if ( $r2 !== false && mysqli_num_rows($r2) > 0 ) 
{
	...
	$desc = test_input($a['descripcion']);
}
?>
<script type="text/javascript">
function goBack(){
	location.href="banner_actualizar.php";
}
</script>
<!-- InstanceEndEditable -->
...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-casa.jpg" width="35" height="24">Editar Banner</h1>
        </article>
        <article id="contenido-admin">
        	<form method="post" enctype="multipart/form-data" name="formulario" id="formulario" onReset="return goBack();">
            	  <input name="id_banner" value="<?php echo $id_banner;?>" type="hidden" class="campo" id="orden">
...
<textarea class="campo" name="descripcion" rows=""><?php echo $desc; ?></textarea>
                
                <h2 class="subtitulos">Adjuntar imagen:</h2>
            	<input type="file" name="imagen" id="imagen"><br>
                
                
                <figure id="img"><img src="../img/banner/<?php echo $ima;?>" width="378" height="144"></figure>
...
$rutaNueva="../img/banner/";

if($nombrefile!="")
{

if ( (strpos($tipo_Imagen, "jpeg") || strpos($tipo_Imagen, "png") || strpos($tipo_Imagen, "jpg") || strpos($tipo_Imagen, "ico") || strpos($tipo_Imagen, "gif")))
{ 

if (file_exists("../img/banner/" . $ima))
			{
				unlink($rutaNueva.$ima);
			}	
	copy($_FILES['imagen']['tmp_name'], $rutaNueva.$nombrefile);
	mysqli_select_db($con_admin, $database_con_admin);
			$qry44 = "UPDATE banner SET orden='$orden',titulo='$titulo',descripcion='$descripcion', imagen='$nombrefile' WHERE id_banner='$id_banner'";
...
else
{
	...
	location.href="banner_editar.php?id_banner='.$id_banner.'";
	</SCRIPT>';	
}
}
else
	{
		mysqli_select_db($con_admin, $database_con_admin);
			$qry44 = "UPDATE banner SET orden='$orden',titulo='$titulo',descripcion='$descripcion' WHERE id_banner='$id_banner'";
			mysqli_query($con_admin, $qry44) or die(header('location:home.php'));
			echo"<script language='javascript'>window.location='banner_actualizar.php'</script>";
	}	
