...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-banner.jpg" width="35" height="24">Usuarios</h1>
        </article>
        
        	<!--<h2 class="subtitulos">Nueva imagen:</h2>            
            <button class="agregar" onclick="location.href='agregar_banner.php'">Agregar</button>-->
            
        </article>
       	
        <article id="contenido-admin">
            <h2 class="subtitulos">Nuevo usuario:</h2>
            <button class="agregar" onclick="location.href='usuario_agregar.php'">Agregar</button>
        </article>
...
<?php
mysqli_select_db($con_admin, $database_con_admin);
$query = "SELECT COUNT(id_usuario2) FROM usuario2";
...
$pages2->mid_range = 20; //cantidad de links a mostrar numero impar 
$pages2->items_per_page = 20; //cantidad de registros por pagina
$pages2->paginate(); 
...
<?php              
mysqli_select_db($con_admin, $database_con_admin);
$qry2 = "SELECT * FROM usuario2 ORDER BY usuario ASC $pages2->limit";
$r2 = mysqli_query($con_admin, $qry2) or die(header('location:home.php'));
if ( $r2 !== false && mysqli_num_rows($r2) > 0 ) 
{
	while ($a2 = mysqli_fetch_assoc($r2))
	{
		$id_usuario2 = test_input($a2['id_usuario2']);
		$usuario = test_input($a2['usuario']);
		$fecha = test_input($a2['fecha']);
?>                    
                    <div id="bloque-actualizar">
                        <div id="bloque-imagen" style="vertical-align:middle;">
                            <p class="texto" style="vertical-align:middle; "><strong>usuario: </strong><?php echo $usuario;?> <br><strong>fecha: </strong><?php echo $fecha?></p>
                        </div>
                        <div id="bloque-acciones" style="vertical-align:middle;">
                            <button class="actualizar" onclick="location.href='usuario_editar.php?id_usuario2=<?php echo $id_usuario2;?>'">Actualizar</button>
                            
                          <button class="eliminar" onclick="if (confirm('¿Desea eliminar el usuario <?php echo $usuario; ?>?')==true){window.location.href='usuario_eliminar.php?id_usuario2=<?php echo $id_usuario2; ?>'}">Eliminar</button>
