...
$qry2 =("SELECT * FROM servicios WHERE id_servicios='$id_servicios'");
$r2=mysqli_query($con_admin, $qry2) or die(header('location:home.php'));
if ( $r2 !== false && mysqli_num_rows($r2) > 0 ) 
{
	$a = mysqli_fetch_assoc($r2);
	$orden = test_input($a['orden']);
	$ima = test_input($a['imagen']);
	$titulo = test_input($a['titulo']);
	$subtitulo = test_input($a['subtitulo']);
	$descripcion = stripslashes($a['descripcion']);
}
?>
<script type="text/javascript">
function goBack(){
	location.href="servicios_actualizar.php";
}
</script>
...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-casa.jpg" width="35" height="24">Editar servicios</h1>
        </article>
        <article id="contenido-admin">
        	<form method="post" enctype="multipart/form-data" name="formulario" id="formulario" onReset="return goBack();">
            	
               <h2 class="subtitulos">Orden:</h2>
                <input name="orden" value="<?php echo $orden;?>" type="text" class="campo" id="orden">
                <input name="id_servicios" value="<?php echo $id_servicios;?>" type="hidden" class="campo" id="orden">
                 <input name="ima" value="<?php echo $ima;?>" type="hidden" class="campo" id="orden">                
                
                 <h2 class="subtitulos">Tìtulo:</h2>
                <input name="titulo" type="text" class="campo" value="<?php echo $titulo; ?>" id="orden">
                
                <h2 class="subtitulos">Descripción:</h2>
<textarea class="mceEditor" name="descripcion" rows=""><?php echo $descripcion; ?></textarea>
                
                <h2 class="subtitulos">Adjuntar imagen:</h2>
            	<input type="file" name="imagen" id="imagen"><br>
                
                
                <figure id="img"><img src="../img/servicios/<?php echo $ima;?>" width="597"></figure>

        		<article id="contenido-admin2">
        			<input name="cancelar" type="reset" class="cancelar" id="cancelar" value="Cancelar" />
            		<input name="guardar" type="submit" class="guardar" id="guardar" value="Guardar" />
        		</article>
            </form>    
        </article>
...
if (isset($_POST['guardar']) ) 
{
		...
		$subtitulo = test_input($subtitulo);
...
if (file_exists("../img/servicios/" . $ima))
			{
				unlink($rutaNueva.$ima);
			}	
	copy($_FILES['imagen']['tmp_name'], $rutaNueva.$nombrefile);
	mysqli_select_db($con_admin, $database_con_admin);
			$qry44 = "UPDATE servicios SET orden='$orden', titulo='$titulo', descripcion='$descripcion', imagen='$nombrefile' WHERE id_servicios='$id_servicios'";
			mysqli_query($con_admin, $qry44) or die(header('location:home.php'));
...
else
{
	...
	location.href="servicios_editar.php?id_servicios='.$id_servicios.'";
	</SCRIPT>';	
}
}
else
	{
		mysqli_select_db($con_admin, $database_con_admin);
			$qry44 = "UPDATE servicios SET orden='$orden', descripcion='$descripcion', titulo='$titulo' WHERE id_servicios='$id_servicios'";
			mysqli_query($con_admin, $qry44) or die(header('location:home.php'));
			echo"<script language='javascript'>window.location='servicios_actualizar.php'</script>";
	}	
