...
?>
    ...
    <section id="contenido2">
    	<article id="titulo">
        	<h1 class="h2"><img src="imagenes/interiores/icono-casa.jpg" width="35" height="24">Agregar servicios</h1>
        </article>
        <article id="contenido-admin">
        	<form method="post" enctype="multipart/form-data" name="formulario" id="formulario" onReset="return goBack();">
                ...
                <input name="orden" type="text" class="campo" id="orden">
                
                 <h2 class="subtitulos">Tìtulo:</h2>
                <input name="titulo" type="text" class="campo" id="orden">
                
                 <h2 class="subtitulos">Descripción:</h2>
<textarea class="mceEditor" name="descripcion" rows=""></textarea>
...
{
	extract ($_REQUEST);
	echo"<script language='javascript'>window.location='actualizar_servicios.php'</script>";
}
...
if (isset($_POST['guardar']))
{
	...
	if ($nombre_archivo4!="")
	{ 
		if ( (strpos($tipo_archivo4 , "jpeg") || strpos($tipo_archivo4 , "png") || strpos($tipo_archivo4 , "gif")) && ($tamano_imagen < 61400400) ) 
		{
			copy($_FILES['imagen']['tmp_name'], $carpeta.$nombre_archivo4);
			extract($_REQUEST);
			mysqli_select_db($con_admin , $database_con_admin);
			$qry44 = "INSERT INTO servicios (imagen, titulo, orden, descripcion) VALUES ('$nombre_archivo4', '$titulo', '$orden', '$descripcion')";
			mysqli_query($con_admin, $qry44) or die(header('location:home.php'));
			echo "<SCRIPT LANGUAJE='javascript'>window.location='servicios_actualizar.php' </SCRIPT>";	
		}
		else
		{  
			echo "<script languaje='javascript'>alert('ERROR EN LA IMAGEN')</script>";
			echo"<script language='javascript'>window.location='servicios_agregar.php'</script>";	
		}	
	}
	else
	{
